...
namespace Urho3D
{
	...
	ValueAnimationInfo::ValueAnimationInfo(const ValueAnimationInfo &other) :
		...
		lastScaledTime_(other.lastScaledTime_)
	{
	}
	...
	void
	ValueAnimationInfo::GetEventFrames(float beginTime, float endTime, PODVector<const VAnimEventFrame *> &eventFrames)
	{
		switch(wrapMode_)
		{
			case WM_LOOP:
				//origin: \todo This can miss an event if the deltatime is exactly the animation's length
				//me: if delta time is zero, ie endTime == beginTime, this is must be at least on circle
				//cause the time is passed!
