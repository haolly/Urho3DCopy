...
namespace Urho3D
{
			...
			if(parent_)
			{
				...
				switch (GetHorizontalAlignment())
				{
					case HA_LEFT:
						break;
					case HA_CENTER:
						//todo, pos 是以 pivot 来计算的还是左上角？ 如果 floatPosition 保存的是原来的值，那么这样计算是不是就有问题？
						// 为啥要这样计算？ 跟 parent 的 size 有啥关系？
	...
	void Sprite::GetBatches(PODVector<UIBatch> &batches, PODVector<float> &vertexData, const IntRect &currentScissor)
	{
		...
		batch.AddQuad(GetTransform(), 0, 0, size.x_, size.y_, imageRect_.left_, imageRect_.top_, imageRect_.right_ - imageRect_.left_, imageRect_.bottom_ - imageRect_.top_);
	...
	{
		Vector3 floatPos((float)position.x_, (float)position.y_);
